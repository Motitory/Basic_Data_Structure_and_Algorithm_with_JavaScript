...
// g - global search
// m - multiline search
// search() : search for a match -> return index
// match() : search for a match -> return String
// exec() : search for a match -> return First match
// test() : search for a match -> return true/false
// ------------------------------------------------------------
// regular expression rule
// 1. ^ : start of string
// 2. \d : any digit
// 3. [abc] : any character between the brackets
// 4. [^abc] : any character except a, b, or c
// 5. [0-9] : any digit between the brackets
// 6. [^0-9] : any digit except between the brackets
// 7. (x|y) : either x or y
// ------------------------------------------------------------
...
// 1. /\d+/ : matches one or more digits (includes 0-9)
var reg = /\d+/;
reg.test("123"); // true
reg.test("33asd"); // true
reg.test("5asdasd"); // true
reg.test("asdasd"); // false
// 2. /^\d+$/ : matches one or more digits (includes 0-9) and nothing else
var reg = /^\d+$/;
reg.test("123"); // true
reg.test("123a"); // false
reg.test("11a22"); // false
// 3. /^[0-9]*.[0-9]*[1-9]+$/ : matches one or more digits (includes 0-9) and nothing else -> for number, float number
var reg = /^[0-9]*.[0-9]*[1-9]+$/;
reg.test("12"); // true
reg.test("12.2"); // true
reg.test("12.2.3"); // false
// 4. /^[a-zA-Z0-9]+$/ : includes number and alphabet
var reg = /[a-zA-Z0-9]/;
...
// 3. e is typically 3, can choose bigger than 2
// 4. d is the value of (e * d) % phi = 1
// Encryption: c = m^e % n -> c is cipher text, m is message
// Decryption: m = c^d % n -> m is message, c is cipher text
// ------------------------------------------------------------
// calculate d
